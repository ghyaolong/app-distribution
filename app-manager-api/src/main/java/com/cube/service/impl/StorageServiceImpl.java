...
import com.cube.service.StorageService;
import com.cube.upload.param.MultipartFileParam;
import com.cube.utils.FileMD5Util;
...
public class StorageServiceImpl implements StorageService {
    ...
    public StorageServiceImpl(@Value("${breakpoint.upload.dir}") String location) {
        if(StringUtils.isEmpty(location)){
            this.rootPaht = Paths.get(System.getProperty("java.io.tmpdir"));
        }
    ...
    private boolean checkAndSetUploadProgress(MultipartFileParam param, String uploadDirPath) throws IOException {
        ...
        if (isComplete == Byte.MAX_VALUE) {
            stringRedisTemplate.opsForHash().delete(Constants.FILE_UPLOAD_STATUS,param.getMd5());
            if(confFile.exists()){
                confFile.delete();
            }
